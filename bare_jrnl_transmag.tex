...
\author{\IEEEauthorblockN{ YiMing Yu,
Cyrill B. Muratov, and
Richard O. Moore}
\IEEEauthorblockA{New Jersey Institute of Technology, Newark, NJ 07102, USA}% <-this % stops an unwanted space
...
\begin{abstract}
Spin-transfer torque magnetoresistive random access memory is a potentially transformative technology in the non-volatile memory market. Their viability depends, in part, on the ability to predictably induce or prevent switching; however,
thermal fluctuations induce small but important errors in both the writing and reading processes.  Computing these very small probabilities for coupled systems of nanomagnetic domains using naive Monte Carlo simulations is impossible due to their slow statistical convergence, but variance reduction techniques can offer an effective way to improve their efficiency.  Here, we apply importance sampling to estimate the read and write soft error rates of macrospin and coupled-spin systems with the easy axis parallel to the film plane.
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
rare event, importance sampling, spin-transfer torque, macrospin magnetic, micromagnetics
\end{IEEEkeywords}}
